...
//		    envVersion: 'develop',  //这个参数是自己加的，文档中没有，但是也可以设置
		    success: function (res) {
//		    	that.logn('打开成功', {})
		        //如果是函数，就去调用 // 打开成功
			 	if(typeof callback == 'function') {
    ...
    setShare: function() {
    	...
    	var bmsControl = that.data.bmsControl
    	//如果是红包，礼包，宝箱，才开启分享图，否则用原来的
    	if(bmsControl.red || bmsControl.energy || bmsControl.box) {
    		type = 'esp'
    	}
    	var shareContent = that.shareTitleImg(type)
    	return shareContent
    },
    //打开分享的方法
    openShare: function(obj) {
    	var that = this
    	var shareContent = that.setShare()
    	log('-分享内容-', shareContent)
    	
    	swan.openShare({
		  title: shareContent.title,
		  content: shareContent.flag,
    ...
    shareFail: function() {
    	var that = this
    	var result = ['请分享到不同的群喔~', '别总打扰这个群嘛，换个试试~', '已经分享过这个啦，换个试试吧~']
    	//随机取一个值返回
		var fails = that.GetArrTarge(result)
		return fails
    },
    //设置用户的值
    setUser: function(page) {
    	var that = this
    	page.setData({
    ...
    //19.2.27新增——播放背景音乐
    playBackgroundAudioManager: function(src) {
    	if(!src) {
    		return
    	}
