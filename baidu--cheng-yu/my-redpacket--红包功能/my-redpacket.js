...
//		  		/获取用户的信息
  	...
  	showRedpacketFirst: function(obj) {
  		...
  		that.setData({
  			...
  			noNext: obj.noNext || false,//关闭时，为true，不需要去到下一题
  		})
...
//		  		var oldValue = app.data.userInfo.balance
//		  		var balance = content.getRedRange(oldValue)
					var balance = app.content.getRedRange()
		  		that.setData({
		  			balance,
		  			game_state: 2,
		  		})
...
//		  	保存用户的能量值
		  	save: function(balance) {
		  		var self = this
		  			//这里去存在本地去
//		  		/获取用户的信息
				    var prom1 = app.apiAsk('BALANCE_API', {
				    	type: 'add', //增加能量值
				    	value: balance,
//								type: 'reduce', //减少能量值
//						    	value: 1,
